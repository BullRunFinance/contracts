...
contract("Masterchef with NFTs", ([owner, investor, referrer]) => {
  ...
  describe("Masterchef transactions", async () => {
    ...
    it("deposit", async() =>{
      ...
      assert.equal((await bullNFT.getBoost((await masterchef.userInfo(0,owner))[4])).toString(), bullseye)
      assert.equal(((await masterchef.userInfo(0,owner))[4]).toString(), nftId)
      ...
      assert.equal((await bullNFT.getBoost((await masterchef.userInfo(1,investor))[4])).toString(), bullFarmer)
      assert.equal(((await masterchef.userInfo(1,investor))[4]).toString(), nftId)
      assert(await bullNFT.ownerOf(nftId), masterchef.address)
...
//      console.log(toEth(await rewardDistribution.assignedRewards()))
      ...
      console.log("NFT bonus: " + (await bullNFT.getBonus((await masterchef.userInfo(1, owner))[4])).toString())
      previousBalanceHolded = toEth(await busdToken.balanceOf(owner))
      ...
      console.log("NFT bonus: " + (await bullNFT.getBonus((await masterchef.userInfo(1, investor))[4])).toString())
      previousBalanceHolded = toEth(await busdToken.balanceOf(investor))
      previousBullBalance = toEth(await bullToken.balanceOf(investor))
